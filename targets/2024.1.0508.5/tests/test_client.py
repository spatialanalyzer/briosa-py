...
def test_protocol_identity_matches_reviewed_compatibility_artifact() -> None:
    assert ARTIFACT_NAME == "briosa-protocol-0.8.0-dev.1-sa-2024.1.0508.5"
    assert SOURCE_REVISION == "3306d43253a1e4e41b75b83360ad4f6f2b7f60b7"
...
def test_wave_a_surface_matches_the_published_capability_set() -> None:
    assert len(WAVE_A_OPERATIONS) == 462
    assert len({item[3] for item in WAVE_A_OPERATIONS}) == 462
...
def test_wave_b_surface_matches_the_published_capability_set() -> None:
    assert len(WAVE_B_OPERATIONS) == 533
    assert len(set(WAVE_B_OPERATIONS)) == 533
    assert len(WAVE_A_OPERATIONS) + len(WAVE_B_OPERATIONS) + 1 == 996
